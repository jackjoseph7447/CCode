...
int read_employees(employeeListE *ls)
{
    ...
    FILE *file = fopen("isisStaff.txt", "r");
    ...
    while(fgets(line, 200, file) != NULL)
    {
        ...
        ls->array[records].id = atoi(strtok(line, "|"));
        strcpy(ls->array[records].name, strtok(NULL, "|"));
        strcpy(ls->array[records].position, strtok(NULL, "|"));
        ls->array[records].salary = atof(strtok(NULL, "|"));
        records ++;
        ls->count += 1;
    }
...
int read_expenses(ExpenseList *el)
{
    ...
    while(fgets(line, 200, file) != NULL)
    {
        ...
        el->array[records].personID = atoi(strtok(line, ","));
        strcpy(el->array[records].description, strtok(NULL, ","));
        el->array[records].cost = atof(strtok(NULL, ","));
...
void calc_expenses(employeeListE el, ExpenseList ep)
{
    ...
    FILE *file = fopen("isisReportJoseph.txt", "w");
